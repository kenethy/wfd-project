...
<script>
    function previewImages(input) {
        console.log('previewImages called with files:', input.files);
        ...
        statusDiv.className = 'col-span-full mb-4 p-3 bg-green-50 border border-green-200 rounded-lg';
        statusDiv.innerHTML = `
        ...
        Array.from(input.files).forEach((file, index) => {
            ...
            reader.onload = function (e) {
                ...
                <img src="${e.target.result}"
                     class="w-full h-24 object-cover rounded-lg border-2 border-gray-300 group-hover:border-blue-400 transition-colors"
                     alt="Preview ${index + 1}">
                <div class="absolute top-1 right-1 bg-black bg-opacity-75 text-white text-xs px-2 py-1 rounded">
                    ${index + 1}
                </div>
                <div class="absolute bottom-1 left-1 bg-green-500 text-white text-xs px-1 py-1 rounded">
                    ✓
                </div>
            `;
    ...
    // Alternative: Add event listener when DOM is ready
